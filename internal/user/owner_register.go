...
func decryptJWE(encryptedJWE string, masterPasswordHash string) (*registerJWEClaims, error) {
	// Decrypt the JWE
	decrypted, err := jwe.Decrypt([]byte(encryptedJWE), jwe.WithKey(jwa.DIRECT(), []byte(masterPasswordHash)))
	if err != nil {
